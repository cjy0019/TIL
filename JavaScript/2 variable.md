...
- <span style=color:skyblue>c언어 같은 언매니지드 언어는 개발자가 명시적으로 메모리를 할당하고 해제하기 위해 `malloc() free()` 와 같은 메모리 제어 기능을 사용한다. </span>
- <span style=color:skyblue>언매니지드 언어는 메모리 제어를 개발자가 주도할 수 있으므로 개발자의 역량에 따라 최적의 성능을 확보할 수 있지만, 치명적인 오류를 낳을 수도 있다.</span>
