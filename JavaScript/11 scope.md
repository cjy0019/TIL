...
스코프는 유효범위라는 의미를 가지고 있고 거의 모든 프로그래밍 언어에서 사용되는 개념이다.
...
<img src="https://poiemaweb.com/assets/fs-images/13-2.png" width="50%"/>

[이미지 출처: poiemaweb](https://poiemaweb.com/fastcampus/scope)
...
위 예제의 bar 함수는 전역에서 정의된 함수다.  생성된 bar 함수 객체는 자신이 정의된 스코프, 즉 전역 스코프를 기억한다. 그리고 bar 함수가 호출되면 호출된 곳이 어디인지 관계없이 언제나 자신이 기억하고 있는 전역 스코프를 상위 스코프로 사용한다. 따라서 위 예제를 실행하면 전역 변수 x의 값 1을 두 번 출력한다.
