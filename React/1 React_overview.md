...
**Virual DOM**에 앞서서 브라우저의 Workflow를 알아야한다.
...
위와 같이 DOM을 조작하면 6가지(간추린)의 일련의 과정이 실행되어 브라우저에 렌더링이 되어진다. 복잡한 SPA(Single Page Application)에서는 DOM 조작이 많이 발생한다. 즉, 그 만큼 브라우저는 많은 연산을 수행하게 되고 전체적인 성능에 영향을 미치게 된다.

이를 해결하기 위해 고안한 것이 **Virtual DOM**이라고 할 수 있다. 뷰에 변화가 생기면 그 변화가 실제 DOM에 적용되기전에 가상의 Virtual DOM에 적용시키고 최종적인 결과를 실제 DOM으로 전달해준다.

**즉, 모든 변화를 묶어서 한번만 실제 DOM에 던져주게 되는 것이다. 결과적으로 리렌더링의 규모는 커지지만 브라우저 내에서 발생하는 연산의 양을 줄였다고 할 수 있다.**
...
아무리 좋은 라이브러리도 단점이 존재하기 마련이다.

1. View-orientedness는 리액트의 큰 단점중 하나이다. 즉 View 이외의 기능은 **써드파티 라이브러리를 이용하거나 직접 구현해야 한다.**
